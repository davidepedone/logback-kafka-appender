...
import ch.davidepedone.logback.kafka.delivery.AsynchronousDeliveryStrategy;
import ch.davidepedone.logback.kafka.keying.NoKeyKeyingStrategy;
...
public class KafkaAppenderIT {
	...
	public final List<Executable> failures = new ArrayList<>();
	...
	public void beforeLogSystemInit() throws Exception {
		...
		loggerContext.getStatusManager().add(status -> {
			if (status.getEffectiveLevel() > ch.qos.logback.core.status.Status.INFO) {
				System.err.println(status);
				if (status.getThrowable() != null) {
					failures.add(() -> org.junit.jupiter.api.Assertions.fail(status.getThrowable()));
				}
				else {
					failures.add(() -> fail(new RuntimeException("StatusManager reported warning: " + status)));
				}
			}
			else {
		...
		unit.setPartition(0);
		unit.setDeliveryStrategy(new AsynchronousDeliveryStrategy());
		unit.addAppender(new AppenderBase<>() {
			@Override
	...
	private final Appender<ILoggingEvent> fallbackAppender = new AppenderBase<>() {
		@Override
		protected void append(ILoggingEvent eventObject) {
			failures.add(() -> fail(new IllegalStateException("Logged to fallback appender: " + eventObject)));
		}
	};
	...
	public void testLogging() {
		...
		assertTrue(unit.isStarted(), "appender is started");
		...
		for (int i = 0; i < messageCount; ++i) {
			...
			final LoggingEvent loggingEvent = new LoggingEvent("a.b.c.d", logger, Level.INFO, sb.toString(), null,
					new Object[0]);
		...
		unit.stop();
		assertFalse(unit.isStarted(), "appender is stopped");
		...
		ConsumerRecords<byte[], byte[]> poll = javaConsumerConnector.poll(Duration.ofMillis(1000));
		int readMessages = 0;
		while (!poll.isEmpty()) {
